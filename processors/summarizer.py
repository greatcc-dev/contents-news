...
[글 구조 - 반드시 이 순서로]
1. 오프닝 후킹 (1~2문장): "이게 무슨 얘기지?" 싶게 만드는 살짝 반전 있는 도입. 자극적 어그로 금지, 뉴스 요약 금지.
2. 문제 또는 관찰 제시 (2~3문장): 독자가 공감할 현실 상황이나 트렌드를 짧게 짚는다.
3. 핵심 인사이트 또는 팁 (①②③ 형식으로 2~3개): 각 항목은 짧은 설명 + 실제 예시나 근거 포함. 숫자·사례·구체적 도구명 적극 활용.
4. 마무리 (1~2문장): 독자에게 행동 의지를 심는 짧고 강한 클로징. 질문형 또는 선언형.

[작성 규칙]
- 전체 1000~1200자 (한국어)
- 이모지 절대 사용 금지
- 해시태그 없음, URL 없음
- 문체: 반드시 존댓말(해요체) 사용 — "~해요", "~습니다", "~거예요", "~세요" 형태로 일관되게. "-다/-라/-거다/-이다"로 끝나는 반말 종결어미 절대 금지
- 어투: 겸손하지만 확신에 찬 세미 캐주얼 (주언규 + GaryVee 스타일 — 독자를 가르치려 하지 않고 함께 생각하는 느낌)
- 핵심 인사이트 각 항목은 충분히 설명할 것: 개념 설명 → 왜 중요한지 → 구체적 상황 예시 → 실행 방법이나 수치·도구명 순서로 3~5문장 이상 작성
